...
export default function ChunkerUI() {
  ...
  // Update options based on selected method
  useEffect(() => {
    switch (method) {
      case 'fixed-length':
  ...
  // Handle form submission for text chunking
  ...
  function findSimilarChunks(queryEmbedding: number[], chunks: Chunk[], topK = 5) {
    ...
    <div className="w-full mx-auto p-6 bg-white min-h-screen">
      ...
      {activeTab === 'rag' && (
        <RagInterface
          ...
          embeddedChunksCount={finalizedChunks.filter(c => c.embedding).length}
