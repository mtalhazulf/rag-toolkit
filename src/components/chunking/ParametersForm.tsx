...
export function ParametersForm({ method, options, setOptions }: ParametersFormProps) {
  return (
      ...
      {method === 'recursive' && (
        <>
          <div className="mb-3">
            <label className="block text-sm font-medium text-black mb-1">
              ...
              <select
                ...
                onChange={(e) => setOptions({ ...options, chunkingMode: e.target.value as 'characters' | 'tokens' })}
                className="mt-1 block w-full rounded-md border border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 p-2 text-sm text-black"
              >
          ...
          <div className="mb-3 p-3 bg-blue-50 rounded-lg">
            <h4 className="text-sm font-medium text-blue-800 mb-1">Recursive Text Splitter</h4>
            <p className="text-xs text-gray-700">
              This advanced chunking method recursively splits text using a hierarchy of separators, starting with larger units (paragraphs) and progressively moving to smaller units (sentences, words) as needed. It creates more natural chunks while respecting content boundaries.
            </p>
            <p className="text-xs text-gray-700 mt-1">
              You can customize the chunk size, overlap, and even provide your own separators in order of precedence.
            </p>
          </div>
          ...
          <div className="mb-3">
            ...
            <p className="text-xs text-gray-500 mt-1">
              Maximum size of each chunk in {options.chunkingMode || 'characters'}
            </p>
          </div>
          ...
          <div className="mb-3">
                ...
                <textarea
                  ...
                  onChange={(e) => {
                    const separators = e.target.value
                      ...
                      .map(s => s.trim())
                      .filter(Boolean);
                    setOptions({ ...options, customSeparators: separators.length > 0 ? separators : undefined });
                  }}
            ...
            <p className="text-xs text-gray-500 mt-1">
              Custom separators to use for recursive splitting (in order of precedence). Leave empty to use defaults.
            </p>
          </div>
          ...
          <div className="mb-3">
            <label className="block text-sm font-medium text-black mb-1">
              ...
              <input
                ...
                className="mt-1 block w-full rounded-md border border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 p-2 text-sm text-black"
                min="0"
                max={options.chunkSize ? Math.max(0, options.chunkSize - 10) : 490}
              />
            </label>
            <p className="text-xs text-gray-500 mt-1">
              Amount of overlap between chunks in {options.chunkingMode || 'characters'}
            </p>
          </div>
        </>
      )}
